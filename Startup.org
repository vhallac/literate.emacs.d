...
*** Reading: notmuch
    ...
    - Get all tags using ~(let ((x nil)) (notmuch-show-mapc (lambda () (push (notmuch-show-get-tags) x))) x)~ or some such
    - Get the union using ~(let ((t nil)) (mapcan lambda (x) x a-list) t)~.
...
** Org mode
   ...
   #+END_SRC
*** Better return key for org-mode

    This implements what is essentially a return-dwim to make sure I don't worry
    about different types of return key for different types of outcomes. This is
    really good for me, as I keep forgetting which modifier does what.

    Return key with a prefix arg, or double return [erforms the usual =org-return=.

    Blog post talking about the code below: [[http://irreal.org/blog/?p=6131][A Better RETURN for Org]] (Irreal)

    #+name: org-support-funcs
    #+BEGIN_SRC emacs-lisp
